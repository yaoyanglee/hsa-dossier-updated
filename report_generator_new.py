...
class ReportGenerator:
    def __init__(self):
        ...
        self.blob_config = self.config["azure_blob"]
        ...
        self.CONNECTION_STRING = self.blob_config["connection_string"]
        self.CONTAINER_NAME = self.blob_config["container_name_docs"]
        self.CONTAINER_NAME_IMAGES = self.blob_config["container_name_images"]

        # Read pickle file of generated assessments
        self.obj = pd.read_pickle(r'assessment_reports/report.pkl')

        # Get project name
        self.project_name = self.get_project_name(self.obj)
        self.logger.info(f"Project name is {self.project_name}")

        self.mapping = {'SN_1': 'Input Data and Output Result',
        ...
        self.container_client = self.blob_service_client.get_container_client(
            self.CONTAINER_NAME)
        self.container_client_images = self.blob_service_client.get_container_client(
            self.CONTAINER_NAME_IMAGES)
    ...
    def get_table_client(self, table_service_client, table_name):
        return table_service_client.get_table_client(table_name)

    def retrieve(self, table_service_client, table_name, project_name, query_key, query_value):
        table_client = self.get_table_client(table_service_client, table_name)
                ...
                if len(match) > 0:
                    ...
                    if len(b) > 0:
                        ...
                        file_match = re.findall(
                            citation_number_pattern, citation_doc)
    ...
    def dehash(self, file):
        ...
        # find the hash
        hash_pattern = rf'{self.project_name}-([a-zA-Z0-9]{{8}})-(.*?)-(.*)'
        ...
        try:
            if len(match) > 0:
                ...
                h_d_map = self.retrieve(azure_table_client.table_service_client,
                                        table_name, self.project_name, query_key, hash_value)
                original_document_name = h_d_map['doc_name']
                # print(original_document_name)

                # replace hash with original file name
                original_chunk_name = f'{self.project_name}-{original_document_name}-{remaining_value}'
    ...
    def process_pkl_to_excel(self, excel_data):
        citation_number_pattern = r'(\[\d+\]).*'
        assessment_pattern = r'(.*?)_assessment'

        with alive_bar(len(self.obj), title="Processing pkl data to excel data", force_tty=True) as bar:
            for criterion, d in self.obj.items():
                self.logger.info(f"Processing items in {criterion}.")
                ...
                for key in d.keys():
                    ...
                    if len(match) > 0:
                        ...
                        dup_ori_cite_list = [self.dehash(cite)
                                             for cite in dup_hash_cite_list]
                        ...
                        for file in dup_hash_cite_list:
                            file_match = re.findall(
                                citation_number_pattern, file)
                            start_index = len(file_match[0]) + 1

                            if file[start_index:] not in unique:
                                hash_file_name = file[start_index:]
                                ori_file_name_with_citation = self.dehash(file)
                                ori_file_name = ori_file_name_with_citation[start_index:]

                                # create a tuple so that can retrieve blob from hash file name and reflect in excel the original file name
                                unique.add((hash_file_name, ori_file_name))
    ...
    def generate_excel(self, excel_data):
        ...
        for a in self.obj['SN_1'].keys():
            ...
            if len(match) > 0:
                ...
                v_d_map = self.retrieve(azure_table_client.table_service_client,
                                        table_name, self.project_name, query_key, vector_store_name)

                # write actual doc names as headers
                header_title = v_d_map['doc_name']
                headers.append(f'{vector_store_name}: {header_title}')
                overview_worksheet.write(
                    0, (header_count+1), f'{vector_store_name}: {header_title}', bold)
                self.logger.info(
                    f"Wrote header '{vector_store_name}: {header_title}' to excel.")
                header_count += 1
        ...
        overview_worksheet.conditional_format(
            1, 1, 7, (header_count+1), {'type': 'no_errors', 'format': border_format})
        ...
        with alive_bar(len(excel_data), title="Writing to excel", force_tty=True) as bar:
            for i, (k, v) in enumerate(excel_data.items()):
                ...
                for header in headers:
                    detailed_worksheet.write(
                        1, (headers.index(header)), header, bold)
                ...
                for key in v.keys():
                    ...
                    if len(citation_match) == 0:
                        self.logger.info(f"Writing response: {k}, {key}.")
                        overview_worksheet.write(
                            i+1, 1+reponse_col_count, v[key], response_format)
                        if key != 'summary':
                            detailed_worksheet.write(
                                2, reponse_col_count-1, v[key], response_format)
                        reponse_col_count += 1

                    # else if citation list, then retrieve from blob and append to excel
                    elif len(citation_match) > 0:
                        self.logger.info(f"Writing citations: {k}, {key}.")
                        ...
                        for n, item in enumerate(v[citation_k]):
                            if '_gen_desc' in item[0]:
                                ...
                                try:
                                    ...
                                    blob_client = self.container_client_images.get_blob_client(
                                        fig_path)
                                    ...
                                    # rows start from index 0. set the row height based on pixels units instead of excel character units (to fit the image)
                                    detailed_worksheet.set_row_pixels(
                                        n+3, cell_height)

                                    x_scale = cell_width/width
                                    y_scale = cell_height/height

                                    detailed_worksheet.insert_image(
                                        n+3, citation_col_count-1, f'{output_path}', {'x_scale': x_scale, 'y_scale': y_scale})
                                except:
    ...
    def generate_report_run(self):
        # Get data to write to excel
        excel_data = {}
        self.process_pkl_to_excel(excel_data)
        ...
        self.generate_excel(excel_data)
